...
INPUT ID Gender$ Ever_Married$ Age Graduated$ Profession$ Work_Experience Spending_Score$ Family_Size Var_1$ Car$;
RUN;
...
data Precio_Auto(keep=Car  Selling_Price Profession );
      set Ventas;
run;
...
proc sort data=Ventas output=Precio_Mas_Menos;
     by ID descending Selling_Price;
run;
